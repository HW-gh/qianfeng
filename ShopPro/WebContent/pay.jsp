...
<body>
	<script type="text/javascript">
		$(function() {
			...
			$("ul[class='op_express_delivery_hot'] li").each(
					function() {
						$(this).click(
								function() {
									$("ul[class='op_express_delivery_hot'] li")
											.removeClass("selected");
									$(this).addClass("selected");
									var send = $(this).find("span").attr("id");
									$("#express").val(send);
								})
					});
										...
										.click(
												function() {
													...
													$
															.ajax({
																type : "GET",
																url : "ShopCartServlet?action=update&count="
																		+ count
																		+ "&id="
																		+ id
																		+ "&tag=frompay",
		...
		/* 		function pay(type, obj) {
		 ...
		 document.getElementById("express").value = type;
		 var x = document.getElementsByClassName("pay-logis");
		 var y = document.getElementsByClassName("OP_LOG_BTN");
		 for (var i = 0; i < x.length; i++) {
		 x[i].innerHTML = type;
		 }
	...
	<div class="concent">
		...
		<div class="paycont">
					...
					<c:forEach items="${addressList }" var="address">
						...
						<div class="address-left">
							<div class="user DefaultAddr">
								<span class="buy-address-detail"> <span class="buy-user">${address.shouhuoren }</span>
							...
							<span class="buy-line-title buy-line-title-type">收货地址：</span> <span
								class="buy--address-detail">${address.address }</span>
						</div>
						<c:if test="${address.isdefault=='是' }">
			...
			<div class="new-addr-btn">
				...
				<c:if test="${address.isdefault!='是' }">
					<a
						href="<%=request.getContextPath()%>/AddressServlet?action=update&id=${address.id}">设为默认</a>
				</c:if>
		...
		<div class="concent">
			<div id="payTable">
				...
				<div class="clear"></div>
				<c:forEach items="${ShopCart.gIList }" var="good">
					<tr class="item-list">
											...
											<div class="item-price price-promo-promo">
												<div class="price-content">
													<div class="price-line">
														<em class="price-original">${good.goods_price }</em>
													</div>
													<div class="price-line">
											...
											<div class="item-amount ">
												...
												<div class="sl">
													<input class="min am-btn" name="" type="button" value="-"
														id="jian" /> <input class="text_box" id="count${good.id}"
														type="text" value="${good.count}"
									...
									<li class="td td-sum">
										<div class="td-inner">
											<em tabindex="0" class="J_ItemSum number"
												id="singlePrice${good.id}">${good.getDanPrice() }</em>
										</div>
									</li>
				...
				<div class="pay-confirm clearfix">
					<div class="box">
						<div tabindex="0" id="holyshit267" class="realPay">
							<em class="t">实付款：</em> <span class="price g_price "> <span>¥</span>
								<em class="style-large-bold-red " id="J_ActualFee">${ShopCart.getTotalPrice() }</em>
							</span>
						</div>
						...
						<div id="holyshit268" class="pay-address">
							...
							<p class="buy-footer-address">
								<span class="buy-line-title buy-line-title-type">寄送至：</span> <span
									class="buy--address-detail" id="send">${address.address }
								</span>
							</p>
							<p class="buy-footer-address">
								<span class="buy-line-title">收货人：</span> <span
									class="buy-address-detail"> <span class="buy-user"
									id="person">${address.shouhuoren }</span> <span
									class="buy-phone" id="tel">${address.phone }</span>
								</span>
							</p>
						</div>
					</div>
					...
					<div id="holyshit269" class="submitOrder">
						...
						<form action="OrderServlet?action=add" method="post" id="form">
							<input type="hidden" name="shouhuoren"
								value="${address.shouhuoren }" id="buy_user" /> <input
								type="hidden" name="phone" value="${address.phone }" id="phone" />
							<input type="hidden" name="address" value="${address.address }"
								id="address" /> <input type="hidden" name="express" value=""
								id="express" /> <input type="hidden" name="bank" value=""
								id="paytype" />
						</form>
					</div>
					<div class="clear"></div>
